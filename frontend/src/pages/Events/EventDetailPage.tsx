...
/**
 * Crea y envía un formulario dinámico para redirigir a PayU.
 * @param {string} payUUrl - La URL de la pasarela de PayU (sandbox o prod)
 * @param {object} formData - El objeto con todos los campos (merchantId, signature, etc.)
 */
function postToPayU(payUUrl: string, formData: any) {
  // 1. Crea un formulario en la memoria
  const form = document.createElement('form');
  form.method = 'POST';
  form.action = payUUrl;

  // 2. Añade cada dato de 'formData' como un input oculto
  ...
  // 3. Añade el formulario al DOM, envíalo y luego elimínalo
...
const EventDetailPage = () => {
  ...
  const handleCheckout = async () => {
    ...
    if (!selectedTicket) {
      setError('Por favor, selecciona un tipo de boleta.');
      return;
    }
    ...
    try {
      // --- PASO 1: CREAR LA BOLETA (Esto funciona bien) ---
      const buyResponse = await axios.post(
        `${import.meta.env.VITE_API_URL}/events/${id}/buy/`,
        {
          config_type_id: selectedTicket.id,
          amount: quantity,
        },
        {
      ...
      const newTicketId = buyResponse.data.ticket_id;
      if (!newTicketId) {
        throw new Error("La API /buy/ no devolvió un ticket_id.");
      }

      // --- PASO 2: INICIAR EL PAGO ---
      const payResponse = await axios.post(
        `${import.meta.env.VITE_API_URL}/payments/ticket/${newTicketId}/pay/`,
        {},
        {
          headers: { Authorization: `Token ${token}` },
        }
      ...
      // 1. La respuesta completa es el formulario de datos
      const formData = payResponse.data;

      // 2. La API no nos dice la URL de PayU, así que la definimos nosotros
      //    basándonos en el campo 'sandbox' que SÍ viene en la respuesta.
      const payuUrl = formData.sandbox
        ? 'https://sandbox.checkout.payulatam.com/ppp-web-gateway-payu/'
        : 'https://checkout.payulatam.com/ppp-web-gateway-payu/';

      // 3. Validamos que el formData tenga campos clave
      if (!formData || !formData.merchantId || !formData.signature) {
        throw new Error("La API /pay/ no devolvió los datos de PayU.");
      }

      // --- PASO 3: REDIRIGIR A PAYU ---
      postToPayU(payuUrl, formData); // Usamos las variables correctas
                        ...
                        >
                          ...
                          {isAvailable && (
                            <div className="w-full bg-gray-200 rounded-full h-1.5">
                              <div
                                className={`h-1.5 rounded-full transition-all ${isAlmostSoldOut ? 'bg-orange-500' : 'bg-brand-500'
                                  }`}
